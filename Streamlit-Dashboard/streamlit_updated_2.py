...
class VideoStreamer:
    def __init__(self):
        ...
        self.frame_queue = queue.Queue(maxsize=2)  # Reduced queue size for faster refresh
    ...
    def _stream_frames(self):
        while self.is_streaming and self.cap.isOpened():
            ...
            if ret:
                ...
                # Clear queue if full to always get latest frame
...
def send_frame_to_api(frame):
    ...
    try:
        ...
        response = requests.post(f"{FASTAPI_URL}/upload_frame", files=files, timeout=5)
...
def main():
    ...
    if 'vision_prediction_counter' not in st.session_state:
        st.session_state.vision_prediction_counter = 0
    ...
    with st.sidebar:
        ...
        vision_analysis_freq = st.slider("Vision Analysis Frequency", 1, 10, 3, 
                                        help="Analyze every Nth frame to reduce CPU load")
    ...
    with col3:
        ...
        <div class="metric-card">
            ...
            <p><strong>Frames Processed:</strong> {st.session_state.frame_counter}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        vision_label_placeholder = st.empty()
        
        if st.session_state.is_streaming:
            # Get latest frame from video streamer
            ...
            if frame is not None:
                ...
                # Only analyze every Nth frame to reduce load (or on first frame)
                should_analyze = (st.session_state.frame_counter % vision_analysis_freq == 0 or 
                                 st.session_state.vision_prediction_counter == 0)
                ...
                if should_analyze:
                    with st.spinner("Analyzing video..."):
                        ...
                        if prediction.get('status') == 'ok':
                            ...
                            # Update vision prediction counter
                            st.session_state.vision_prediction_counter += 1
                    ...
                    vision_label_placeholder.markdown(f"""
                    <div style="background-color: {label_color};
                        ...
                        <p style="color: white; margin: 0; font-size: 0.8em;">Frame: {st.session_state.frame_counter} | Analysis: {st.session_state.vision_prediction_counter}</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    # Show previous prediction while skipping analysis
                    if st.session_state.last_vision_prediction['label'] != 'none':
                        label_display = st.session_state.last_vision_prediction['label'].replace('_', ' ').title()
                        vision_label_placeholder.markdown(f"""
                        <div style="background-color: #666;
                                    padding: 10px; border-radius: 5px; text-align: center; margin-top: 10px;">
                            <h3 style="color: white; margin: 0;">
                                {label_display} (Cached)
                            </h3>
                            <p style="color: white; margin: 0; font-size: 0.8em;">Skipping analysis this frame ({st.session_state.frame_counter})</p>
                        </div>
                        """, unsafe_allow_html=True)
        else:
            # Show static image when not streaming
            video_placeholder.info("Click 'Start Video Monitoring' to begin video feed")
            vision_label_placeholder.empty()
    ...
    # Auto-refresh for real-time updates - ONLY if streaming or recording
    if st.session_state.is_recording or st.session_state.is_streaming:
        # Set refresh rate based on activity
        refresh_rate = 0.5 if st.session_state.is_streaming else 3.0
